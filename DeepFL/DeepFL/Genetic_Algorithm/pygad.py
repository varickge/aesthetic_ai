...
######## changes on lines 1705, 1709            non duplicate trials changes to 100
######## changes on line 2143                   not all offsprings are mutated
######## changes on line 1765-1776              0, 1 sum numbers remain the same
class GA:
    ...
    def two_points_crossover(self, parents, offspring_size):
        ...
        for k in range(offspring_size[0]):
            ...
            ##############################
            if offspring[k].sum() != parents[0].sum():
                change = offspring[k].sum() - parents[0].sum()
                if change > 0:
                    idx = numpy.nonzero(offspring[k])[0]
                    inx_to_change = numpy.random.choice(idx, size=int(abs(change)), replace=False)
                    offspring[k][inx_to_change] = 0 
                else:
                    idx = numpy.where(offspring[k] == 0)[0]
                    inx_to_change = numpy.random.choice(idx, size=int(abs(change)), replace=False)
                    offspring[k][inx_to_change] = 1
            ##############################
            if (self.mutation_type is None) and (self.allow_duplicate_genes == False):
                if self.gene_space is None:
                    offspring[k], _, _ = self.solve_duplicate_genes_randomly(solution=offspring[k],
    ...
    def mutation_randomly(self, offspring):
        ...
        for offspring_idx in numpy.random.choice(range(offspring.shape[0]), offspring.shape[0]/2, replace=False):
            mutation_indices = numpy.array(random.sample(range(0, self.num_genes), self.mutation_num_genes))
            for gene_idx in mutation_indices:
