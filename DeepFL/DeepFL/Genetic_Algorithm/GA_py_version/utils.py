...
def cycle_for_population(curr_pop, weights_path, path_to_save, model, data, val_data, lbl, val_lbl, bench,  bench_labels, 
        ...
        if acc > 93:
            np.save(f'{path_to_save}best_solution_custom_{acc}', curr_pop)
            model.save_weights(f'{path_to_save}best_custom_{acc}.hdf5')
        print(f'ACC: {acc}')
        return acc
...
def findBestFeats(pop_size, feature_vector_size, transformed_feat_vector_size, num_of_parents=4, max_gen=100, init_size=2,
                  ...
                  train_data_path=f'{generate_root_path()}/Data/AesthAI/alm/splitted/alm_train/',
                  bench_path=f'{generate_root_path()}/Data/AesthAI/benchmark_connected/',
                  feats_MG='max_996',
                  feats_CNN='border_600x600', cnn='cnn_efficientnet_b7'):
    # Creating FC model
    model = fc_model_softmax(input_num=transformed_feat_vector_size)
...
def findBestFeatsPy(pop_path, generation, feature_vector_size, transformed_feat_vector_size, num_of_parents=4,
    ...
    print('bench shapes', bench.shape, bench_labels.shape)
