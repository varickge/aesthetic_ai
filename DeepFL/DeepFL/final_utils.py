...
def extract_features_from_path_automated_json(source_file, target_file, model, label, splitted='',
                                             crop_func=None, resize_func=None, size=None, for_all=False, save_json=False):
    '''
    Takes file split with batch size puts in the deepfl and saves the features in a separate folder
    '''
...
def normalize_feat_vector(feature_vector, 
                          mean_path="D:/Data/AesthAI/alm/splitted/alm_train/features/cnn_efficientnet_b7/norm_vectors/mean.npy", 
                          std_path="D:/Data/AesthAI/alm/splitted/alm_train/features/cnn_efficientnet_b7/norm_vectors/std.npy"):
    
    mean = np.load(mean_path)
    std = np.load(std_path)
    return (feature_vector - mean) / std 

def pca_transform(vector,path = 'models/PCA/PCA_MultiGap_8464_auto.pkl'):
